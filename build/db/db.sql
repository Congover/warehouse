...
CREATE TABLE `tPRODUCT` (
  ...
  CONSTRAINT `FK_PRODUCT_PARENT` FOREIGN KEY(`PARENT_ID`) REFERENCES `tPRODUCT` (`PRODUCT_ID`)
);
...
CREATE TABLE `tPRODUCT_QUANTITY` (
  `PRODUCT_QUANTITY_ID` BIGINT NOT NULL AUTO_INCREMENT,
  `PRODUCT_BAG` INT NULL,
  `PRODUCT_COUNT` DOUBLE NULL,
  `PRODUCT_ID` BIGINT NOT NULL,
  PRIMARY KEY (`PRODUCT_QUANTITY_ID`),
  KEY `IDX_FK_PROD_QUAN_TO_PR` (`PRODUCT_ID`),
  CONSTRAINT `FK_PROD_QUAN_TO_PR` FOREIGN KEY (`PRODUCT_ID`) REFERENCES `tproduct` (`PRODUCT_ID`)
);
...
INSERT INTO tPRODUCT_QUANTITY (`PRODUCT_QUANTITY_ID`, `PRODUCT_ID`, `PRODUCT_BAG`, `PRODUCT_COUNT`) VALUES (1, 1, 0, 0);

//Пользователь
INSERT INTO tUSER_ROLES (`AUTHORITY`) VALUES ('ROLE_USER');
INSERT INTO tUSER (`LOGIN`, `PASSWORD`, `EMAIL`, `USER_ROLES_ID`) VALUES ('admin', 'admin', 'admin', (SELECT USER_ROLES_ID FROM tUSER_ROLES WHERE AUTHORITY = 'ROLE_USER'));
